...
const CACHE_CONFIG = {
  // 缓存开关
  ENABLE_LOCAL_CACHE: process.env.NEWAPI_ENABLE_CACHE !== 'false', // 默认开启
  
  // 缓存阈值配置
  MIN_CONTEXT_TOKENS: 1000,       // 最小上下文token数量才启用缓存
  LARGE_CONTEXT_TOKENS: 20000,    // 大上下文阈值，超过此值强制缓存
  MAX_CACHE_SIZE: 50,             // 最大缓存条目数
  
  // 缓存时间配置
  DEFAULT_TTL: 30 * 60 * 1000,    // 默认30分钟
  LARGE_CONTEXT_TTL: 60 * 60 * 1000, // 大上下文1小时
  
  // 缓存命中策略
  SIMILARITY_THRESHOLD: 0.8,      // 相似度阈值
  HASH_PRECISION: 16,             // 哈希精度
  
  // 调试模式
  DEBUG_CACHE: process.env.NEWAPI_CACHE_DEBUG === 'true'
};
